...
void Score::cmdAddMeasureRepeat(Measure* first, int numMeasures, int staffIdx)
{
    //
    // make measures into group
    //
    if (!makeMeasureRepeatGroup(first, numMeasures, staffIdx)) {
        return;
    }
...
bool Score::makeMeasureRepeatGroup(Measure* first, int numMeasures, int staffIdx)
{
    ...
    for (auto m : measures) {
        ...
        undo(new ChangeMeasureRepeatCount(m, i++, staffIdx));
        if (m != measures.front()) {
